...
- **読むことに専念できる表示** — GFM、Obsidian互換のCallout、脚注、数式、タスクリスト、表、ローカル画像、コードハイライトに対応。ライト／ダークモードにも追従します。
...
| 形式 | 拡張子 | 主な対応内容 |
| --- | --- | --- |
| Markdown | `.md`, `.markdown` | GFM、Obsidian互換のCallout、脚注、インライン／ブロック数式、安全な生HTML |
...
Obsidianと同じ引用ブロック形式でCalloutを記述できます。タイトルのMarkdown、本文のリストやコードブロック、Calloutのネストにも対応しています。
...
> [!warning]- 最初は折りたたむ
> `-`は折りたたみ、`+`は展開した状態で表示します。
```

標準タイプと別名は種類ごとの配色になり、未知のタイプは`note`相当の表示になります。
